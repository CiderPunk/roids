...
use crate::{game::PauseState, input::{InputEventType, InputTriggerEvent}};
 ...
 impl Plugin for PauseScreenPlugin{
  fn build(&self, app: &mut App) {
    app
      ...
      .add_systems(Update, update_pause_screen.run_if(in_state(PauseState::Paused)));
  }
}
...
fn update_pause_screen(
  ...
  for InputTriggerEvent { action, input_type } in ev_shoot_event.read() {
    if *input_type == InputEventType::Pressed{
      info!("Resuming game");
