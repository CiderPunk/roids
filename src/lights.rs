...
fn rotate_lights(
  time:Res<Time>,
  mut query:Query<(&mut RotateLight, &mut Transform)>,
){
  for (mut rotate, mut transform) in query{
    rotate.rotation += time.delta_secs() * rotate.rate;
    transform.translation = Vec3::new( rotate.distance * rotate.rotation.sin(),50., rotate.distance * rotate.rotation.cos());
  }
}
...
fn spawn_lights(mut commands:Commands){
  ...
  commands.spawn((
    ...
    RotateLight{ distance: 200., rotation: 0., rate: 0.2, },
  ));
  ...
  commands.spawn((
    ...
    Transform::from_translation(Vec3::new(100., 50., 100.)),
    RotateLight{ distance: 200., rotation: PI, rate: 0.33, },
  ));
