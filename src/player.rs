...
const PLAYER_SHOOT_DELAY: f32= 0.05;
const PLAYER_BULLET_FORWARD_OFFSET: f32 = 2.5;
const PLAYER_BULLET_VELOCITY: f32 = 40.;
...
impl Plugin for PlayerPlugin{
  fn build(&self, app: &mut App) {
    app
      ...
      .add_systems(Update, (update_player_movement, update_player_action, player_shoot).chain().run_if(in_state(PauseState::Running)));
  }
}
...
fn update_player_movement(
  ...
  mut ship:Single<(&GlobalTransform, &mut Acceleration, &mut Rotation),With<Player>>
){
...
fn player_shoot(
  ...
  if player.next_shoot_time < 0.{
    ...
    ev_shoot_event.write(ShootEvent::new(true, transform.translation() + (forward * PLAYER_BULLET_FORWARD_OFFSET),
     (forward * PLAYER_BULLET_VELOCITY) + velocity.0, PLAYER_BULLET_DAMAGE, PLAYER_BULLET_SCALE, player_entity));
